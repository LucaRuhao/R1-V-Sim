    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                # pixel_values:
                # torch.Size([2992, 1176])
                # image_embeds:
                # torch.Size([748, 1536])
                # image_mask:
                # torch.Size([4, 244, 1536])
                # inputs_embeds:
                # torch.Size([4, 244, 1536])
                # n_image_tokens:
                # 748
                # tokens_per_image: 187

                batch_size = inputs_embeds.shape[0]
                tokens_per_image = image_embeds.shape[0] // batch_size
        ...
        # 恢复 masked_scatter，得到图像文本各自hidden states
        if pixel_values is not None or pixel_values_videos is not None:
            recovered_text_embeds = hidden_states[~image_mask].view(hidden_states.shape[0], -1, hidden_states.shape[-1])
            recovered_image_embeds = hidden_states[image_mask].view(hidden_states.shape[0], -1, hidden_states.shape[-1])
            # print("recovered_image_embeds: \n", recovered_image_embeds.shape)
        else:
            recovered_text_embeds = torch.zeros(hidden_states.shape)
            recovered_image_embeds = torch.zeros(hidden_states.shape)
        ...
        return Qwen2VLCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
            image_embeddings=recovered_image_embeds,
            text_embeddings=recovered_text_embeds,
